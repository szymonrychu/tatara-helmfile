...
The standalone helmfile bucket that owns the tatara platform's helm releases
plus the enrollment CRs (the `tatara` Project and the per-repo Repository CRs in
`values/project-tatara/common.yaml`). It deploys via GitHub Actions on an
in-cluster ARC runner (`arc-runner-tatara-helmfile`) using the pod's in-cluster
ServiceAccount token - no KUBECONFIG. A PR posts a sticky `helmfile diff`; merge
to `main` auto-applies. This intentionally reverses the 2026-06-05
consolidation into the infra helmfile (see `MEMORY.md`) to scope bot deploy
access to one repo.

It also owns the `cd-release` composite action that every tatara repo's release
workflow calls.
...
- The deploy runner SA is cluster-admin scoped - the single highest-risk element
  in the platform. Any code that runs in `arc-runner-tatara-helmfile` can do
  anything to the cluster. Keep this repo bot-only-write and private.
- NEVER `docker buildx` or `helm push` locally; component CI builds and pushes
  to harbor on merge to main.
- `apply-pins.py` is a version-string rewriter: its `value_template` understands
  `{{version}}`, `{{image_version}}` and `{{chart_version_bare}}` and nothing
  else. It is not a general file-content channel.
